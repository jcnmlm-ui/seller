...
export default function OrderConfirm() {
  ...
  // 目前頁面的完整 URL，作為 QR Code 內容
  const qrValue = window.location.href

  return (
    <div className="min-h-screen bg-stone-50 pb-8">
      {/* 成功 Header */}
      <div className={`${isPending ? 'bg-red-500' : 'bg-green-500'} text-white py-8 px-6 text-center`}>
      ...
      <div className="max-w-sm mx-auto px-4 py-5 space-y-4 fade-up">
        ...
        <div className="card p-4 space-y-2">
          ...
          {[
            ...
            <div key={l} className="flex gap-3 text-sm">
              <span className="text-stone-400 w-14 flex-shrink-0">{l}</span>
        ...
        {isPending && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-sm text-amber-800">
            ...
            <p>請截圖保存或加入書籤，至攤位出示後即可完成結帳。若不慎關閉此頁，可使用下方連結重新查詢。</p>
          </div>
        )}
        ...
        <div className="flex gap-3">
          ...
          <Link to="/query" className="btn-ghost flex-1 text-center text-sm">
            查詢訂單
          </Link>
        </div>
      </div>
