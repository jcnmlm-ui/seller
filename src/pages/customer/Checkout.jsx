...
export default function Checkout() {
  ...
  const [form, setForm] = useState({
    name: '', phone: '', postal_code: '', address: '', note: ''
  })
  ...
  function validate() {
    ...
    if (!form.postal_code.trim()) e.postal_code = '請填寫郵遞區號'
    ...
    try {
      ...
      const { data: order, error: orderErr } = await supabase
        ...
        .insert({
          ...
          receiver_postal_code: formatPostalCode(form.postal_code.trim()),
            ...
            {items.map(item => (
              <div key={item.id} className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => dispatch({ type: 'REMOVE', id: item.id })} className="text-stone-300 hover:text-red-400 transition-colors">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
          ...
          <div className="flex gap-3 items-start">
            <div style={{width:'110px',flexShrink:0}}>
              ...
              <input type="text" inputMode="numeric" maxLength={7}
                className={`input font-mono tracking-widest text-center ${errors.postal_code ? 'border-red-400' : ''}`}
                placeholder="800-001" value={form.postal_code}
                onChange={e => { setForm(f=>({...f,postal_code:e.target.value})); setErrors(e2=>({...e2,postal_code:''})) }} />
              {errors.postal_code && <p className="text-red-500 text-xs mt-1">{errors.postal_code}</p>}
            </div>
            <div className="flex-1">
              <label className="label">收件地址</label>
              <input type="text" className={`input ${errors.address ? 'border-red-400' : ''}`}
                placeholder="高雄市新興區中正三路177號" value={form.address}
                onChange={e => { setForm(f=>({...f,address:e.target.value})); setErrors(e2=>({...e2,address:''})) }} />
              {errors.address && <p className="text-red-500 text-xs mt-1">{errors.address}</p>}
            </div>
          </div>
          <p className="text-xs text-stone-400 -mt-2">📮 請填寫新式 6 碼郵遞區號（如 800-001），系統自動加入格式</p>
          ...
          <button onClick={handleSubmit} disabled={loading} className="btn-primary w-full text-base py-4">
            {loading ? (
              <span className="flex items-center justify-center gap-2">
                <span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />送出中…
              </span>
