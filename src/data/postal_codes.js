/**
 * 台灣郵遞區號對照表（3碼）
 * 資料來源：中華郵政股份有限公司 官方郵遞區號資料
 * 驗證網址：https://www.post.gov.tw/post/internet/Postal/index.jsp?ID=208
 * 版本：2024年
 *
 * 使用說明：
 * - 此對照表依「縣市 → 鄉鎮市區 → 3碼郵遞區號」結構組成
 * - 3碼為投遞到鄉鎮市區層級，各大快遞（郵局/黑貓/宅急便）均接受
 * - 新式6碼後3碼依街道投遞路線分配，無法從行政區自動判斷，需手動填寫
 */
...
export const POSTAL_DATA = {
  ...
  '高雄市': {
    ...
    '大社區': '815', '東沙群島': '817', '南沙群島': '819',
    '岡山區': '820', '路竹區': '821', '阿蓮區': '822', '田寮區': '823',
    '燕巢區': '824', '橋頭區': '825', '梓官區': '826', '彌陀區': '827',
    '永安區': '828', '湖內區': '829', '鳳山區': '830', '大寮區': '831',
    '林園區': '832', '鳥松區': '833', '大樹區': '840', '旗山區': '842',
    '美濃區': '843', '六龜區': '844', '內門區': '845', '杉林區': '846',
    '甲仙區': '847', '桃源區': '848', '那瑪夏區': '849', '茂林區': '851',
    '茄萣區': '852',
  },
  '屏東縣': {
...
// 取得所有縣市清單
export const CITIES = Object.keys(POSTAL_DATA)

// 依縣市取得鄉鎮市區清單
export function getDistricts(city) {
  return city ? Object.keys(POSTAL_DATA[city] ?? {}) : []
}

// 依縣市+鄉鎮取得3碼郵遞區號
export function getPostalCode(city, district) {
  return POSTAL_DATA[city]?.[district] ?? ''
}